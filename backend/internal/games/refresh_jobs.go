...
func (manager *RefreshJobManager) Start(userID string) (RefreshJobStatus, bool) {
	...
	if manager.stopped {
		failed := RefreshJobStatus{State: RefreshJobFailed, Message: "Game metadata refresh could not be started."}
...
func (manager *RefreshJobManager) run(userID string) {
	...
	defer manager.mu.Unlock()
	if err != nil {
		manager.jobs[userID] = RefreshJobStatus{State: RefreshJobFailed, Message: "Game metadata refresh could not be completed. Try again."}
		return
	}
