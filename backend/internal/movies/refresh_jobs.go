...
func (service *Service) RefreshAll(ctx context.Context, userID string) (BulkRefreshResult, error) {
	...
	for _, item := range items {
		...
		_, err := service.Refresh(ctx, userID, item.ID)
		if err != nil {
			result.Failed++
...
func (manager *RefreshJobManager) Start(userID string) (RefreshJobStatus, bool) {
	...
	if manager.stopped {
		failed := RefreshJobStatus{State: RefreshJobFailed, Message: "Movie metadata refresh could not be started."}
...
func (manager *RefreshJobManager) run(userID string) {
	...
	defer manager.mu.Unlock()
	if err != nil {
		manager.jobs[userID] = RefreshJobStatus{State: RefreshJobFailed, Message: "Movie metadata refresh could not be completed. Try again."}
		return
	}
